...
public static class PathPointStrategyFactory
{
    ...
    private class AngledDownStrategy : IPathPointStrategy
    {
        public List<PathPointCommand> GetPathPoints(PathCurve pathCurve)
        {
            ...
            pullbackVector *= pathCurve.Line.Length / 2.5;
            ...
            // Path start
            sequence.Add(new PathPointCommand(
                pullbackPointvert, 1.8, coolingOn: true, extrudeOn: true, heatOn: true, velRatio: velRatio, cycleWait: false));

            // Path start
            sequence.Add(new PathPointCommand(
                pullbackPointvert, 1.8, coolingOn: false, extrudeOn: true, heatOn: true, velRatio: velRatio, cycleWait: false));

            // Modified end 
            sequence.Add(new PathPointCommand(
                modifiedEndPoint, 2.2, coolingOn: false, extrudeOn: true, heatOn: true, velRatio: velRatio, cycleWait: false));
            ...
            if (pathCurve.Line.Length < 20.0)
            {
                ...
                sequence.Add(new PathPointCommand(
                    pathEnd, 2.4, coolingOn: true, extrudeOn: false, heatOn: true, velRatio: velRatio, cycleWait: false));
            }
            else if (pathCurve.Line.Length > 50.0)
            {
                pathEnd = new Point3d(pathEnd.X, pathEnd.Y, pathEnd.Z - 1.0);

                sequence.Add(new PathPointCommand(
                    pathEnd, 2.8, coolingOn: true, extrudeOn: false, heatOn: true, velRatio: velRatio, cycleWait: false));
            }
            else
            {
                sequence.Add(new PathPointCommand(
        ...
        public List<PathPointCommand> GetPathPoints(PathCurve pathCurve)
        {
            ...
            if (lineLength > step)
            {
                ...
                for (int i = 1; i <= numSteps; i++)
                {
                    ...
                    Point3d wait_pt2 = new Point3d(wait_pt.X, wait_pt.Y, wait_pt.Z - 2.0);
                    // Decrease E5 value for each step 
                    velRatio = velRatio * 0.5f;   
                    ...
                    sequence.Add(new PathPointCommand(wait_pt, e5 / 2.8, coolingOn: true, extrudeOn: true, heatOn: true, velRatio: velRatio / 2, cycleWait: true));
                }
            }
    ...
    private class HorizontalStrategy : IPathPointStrategy
    {
        ...
        public List<PathPointCommand> GetPathPoints(PathCurve pathCurve)
        {
            ...
            Point3d pathStartEdit = new Point3d(pathStart.X, pathStart.Y, pathStart.Z + 2);

            // move Z + 10
            Point3d pathEndMoveZ = new Point3d(pathEnd.X, pathEnd.Y, pathEnd.Z + 10);
            Point3d pathEndMoveZFinal = new Point3d(pathEnd.X, pathEnd.Y, pathEnd.Z + 3);
